#!/usr/bin/env python3
"""
Lambda Mini Test - Test AWS Lambda endpoints directly
"""
...
ENDPOINTS = {
    'health': 'https://9voqzgx3ch.execute-api.us-east-1.amazonaws.com/prod/healthz',
    'agents': 'https://9voqzgx3ch.execute-api.us-east-1.amazonaws.com/prod/agents',
    'pdf_search': 'https://9voqzgx3ch.execute-api.us-east-1.amazonaws.com/prod/pdf/search',
    'debate_export': 'https://9voqzgx3ch.execute-api.us-east-1.amazonaws.com/prod/debate/export',
    'debate_stream': 'https://9voqzgx3ch.execute-api.us-east-1.amazonaws.com/prod/debate/stream'
}
...
def test_agents():
    ...
    try:
        # GET
        response = requests.get(ENDPOINTS['agents'], timeout=10)
        get_ok = response.status_code == 200 and isinstance(response.json(), list)
        
        # POST
        post_response = requests.post(ENDPOINTS['agents'], 
...
def test_pdf_search():
    """Test PDF search"""
    try:
        response = requests.post(ENDPOINTS['pdf_search'], 
                               json={"query": "test"}, 
                               timeout=10)
        data = response.json()
...
def test_debate_export():
    ...
    try:
        ...
        response = requests.post(ENDPOINTS['debate_export'], 
                               json=payload, 
                               timeout=10)
...
def test_sse_stream():
    """Test SSE stream"""
    try:
        response = requests.get(ENDPOINTS['debate_stream'], 
                              stream=True, 
                              timeout=15)
...
def main():
    ...
    if passed == total:
        print("🎉 All Lambda endpoints working!")
    else:
        print("🔧 Some endpoints need deployment.")
