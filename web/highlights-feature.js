// R1: ENHANCED SMART HIGHLIGHTS FEATURE
// Add this to backup.html before closing </script> tag

// Initialize user highlights storage
if (!state.userHighlights) state.userHighlights = {};

// CSS for highlights (add to <style> section)
...
#selectionToolbar {
    ...
    border-radius: 12px;
    padding: 8px;
    box-shadow: 0 8px 28px rgba(16,185,129,0.2), 0 0 50px rgba(6,182,212,0.12);
    z-index: 1001;
    gap: 4px;
}
...
// HTML for selection toolbar and legend (add before closing </body>)
const highlightHTML = `
...
// Add highlight legend button to right sidebar (add after focusBtn)
const highlightBtnHTML = `<button id="highlightLegendBtn" class="text-xs px-2 py-1 rounded-lg border hover:bg-gray-50" data-tooltip="Highlights" title="Show highlights">🎨</button>`;
...
// Add button handler
document.getElementById('highlightLegendBtn')?.addEventListener('click', toggleHighlightLegend);
