...
def lambda_handler(event, context):
    ...
    for record in event['Records']:
        try:
            ...
            # Store in simple format (skip Pinecone for now, just mark as indexed)
            logger.info("Marking document as indexed (vector storage simulated)")
            ...
            table.update_item(
                ...
                UpdateExpression='SET #status = :status, extractedText = :text, processedAt = :timestamp, indexed = :indexed, embedding = :embedding',
                ...
                ExpressionAttributeValues={
                    ...
                    ':indexed': True,
                    ':embedding': str(len(embedding))  # Just store embedding size for now
                }
            )
